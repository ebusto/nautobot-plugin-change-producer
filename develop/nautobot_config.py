...
cache_ops_pwd = os.getenv("NAUTOBOT_REDIS_PASSWORD", "redis")
...
# These people will be notified of
...
# Set to False to disable caching with cacheops. (Default: False)
...
# If hosting Nautobot in a subdirectory, you must set this value to match the base URL prefix configured
# in your HTTP server (e.g. `/nautobot/`). When not set, URLs will default to being prefixed by `/`.
...
LOGGING = {
    ...
    "formatters": {
        ...
        "verbose": {
            "format": "%(asctime)s03d %(levelname)-7s %(name)-20s %(filename)-15s %(funcName)30s() :\n  %(message)s",
            "datefmt": "%H:%M:%S",
        },
    },
    "handlers": {
